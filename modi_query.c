...
void Create_GF(AdjList*G){
	...
	int flag=0;
	string x,la,y;
	bool whethertype=false;
	bool whetherbegin=false;
	while(cin>>x>>la>>y){
		int len=y.length();
		if(strcmp(la,"type")==0)whethertype=true;
		if(whetherbegin==false){
			cent=x;
			whetherbegin=true;
...
void Modify_GF(AdjList *G){
	if(whethertype==false){//add "type" edge
		EdgeNode*s=new EdgeNode;
		s->valued="type";
		s->adjvex=ed;
		s->next=G->vertices[st].firstedge;
